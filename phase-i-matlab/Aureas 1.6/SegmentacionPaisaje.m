function [datos nombre_archivo FS] = SegmentacionPaisaje(rutain,canal)
...
waitbar(0,Bar,sprintf('%d%% completed...',0));
    ...
    for i=1:length(Dir) %subcarpeta
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            [s,f,t] =spectrogram(y(:,canal),flattopwin(wspec),ovp,2048,FS); %256,128,256
    ...
    %                             Metric2 = strongest.Metric;
                                ...
                                if pend>pini && maxi>mini
                                    ...
                                    datos(por,:)=[Mes;Dia;Hora;Min;pini.*((resiz)/FS);pend.*((resiz)/FS);(pend-pini).*((resiz)/FS);dom.*(FS/2);mini.*(FS/2);maxi.*(FS/2);band_1(j);band_2(j);features];
