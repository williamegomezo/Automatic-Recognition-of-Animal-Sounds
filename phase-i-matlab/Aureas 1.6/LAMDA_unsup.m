function [gadso,recon,mean_clas,std_clas] = LAMDA_unsup(ex_level,it_num,dat_norma,mad,gad)
...
Bar = waitbar(0,'Initializing waitbar...','Name','Classifying');
...
for t =1:it_num
    ...
    for j = 1:num_datos
        switch mad
             ...
             case 'trapz'
             mads = MadBell(dat_norma(j,:),mean_clas,std_clas);
             otherwise
             disp('incorrect GAD')    
        end
        switch gad
...
for j = 1:num_datos
    switch mad
             ...
             case 'gauss'
             mads = exp((-(da_norma(j*ones(size(mean_clas(2:end,:),1),1),:)-mean_clas(2:end,:)).^2)./(2*std_clas(2:end,:).^2));
             case 'trapz'
             mads = MadBell(dat_norma(j,:),mean_clas(2:end,:),std_clas(2:end,:));
             otherwise
             disp('incorrect GAD') 
    end
    switch gad
